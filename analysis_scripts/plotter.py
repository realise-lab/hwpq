...
ARCHITECTURE_STYLES = {
    "register_array": {"color": "blue", "marker": "o", "display_name": "Register Array"},
    "systolic_array": {"color": "green", "marker": "^", "display_name": "Systolic Array"},
    "register_tree": {"color": "red", "marker": "x", "display_name": "Register Tree"},
    "bram_tree": {"color": "purple", "marker": "s", "display_name": "BRAM Tree"},
    "pipelined_bram_tree": {"color": "brown", "marker": "D", "display_name": "Pipelined BRAM Tree"},
    "hybrid_tree": {"color": "orange", "marker": "d", "display_name": "Hybrid Tree"},
...
def get_arch_style(arch_name):
    ...
    return {"color": "gray", "marker": ".", "display_name": arch_name.replace("_", " ").title()}


def setup_plot_style(font_size=None, figsize=None, dpi=None, grid=None, grid_alpha=None):
    """
...
def plot_frequency_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    style = get_arch_style(arch_name) if arch_name else {"color": "blue", "marker": "o", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, frequencies, f"{style['marker']}-", color=style['color'], 
...
def plot_lut_usage_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    # Get architecture-specific style
    style = get_arch_style(arch_name) if arch_name else {"color": "red", "marker": "s", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, luts, f"{style['marker']}-", color=style['color'], 
           linewidth=2, label=style['display_name'])
...
def plot_lut_utilization_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    # Get architecture-specific style
    style = get_arch_style(arch_name) if arch_name else {"color": "green", "marker": "d", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, lut_percentages, f"{style['marker']}-", color=style['color'], 
           linewidth=2, label=style['display_name'])
...
def plot_register_usage_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    # Get architecture-specific style
    style = get_arch_style(arch_name) if arch_name else {"color": "purple", "marker": "^", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, registers, f"{style['marker']}-", color=style['color'], 
           linewidth=2, label=style['display_name'])
...
def plot_register_utilization_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    # Get architecture-specific style
    style = get_arch_style(arch_name) if arch_name else {"color": "orange", "marker": "*", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, reg_percentages, f"{style['marker']}-", color=style['color'], 
           linewidth=2, label=style['display_name'])
...
def plot_bram_utilization_vs_queue_size(ax, data_dict, title=None, arch_name=None):
    ...
    # Get architecture-specific style
    style = get_arch_style(arch_name) if arch_name else {"color": "magenta", "marker": "X", "display_name": "Architecture"}
    
    ax.plot(queue_sizes, bram_percentages, f"{style['marker']}-", color=style['color'], 
           linewidth=2, label=style['display_name'])
...
def plot_performance_comparison(ax, data_dict, arch_list, operation, title=None):
    ...
    for arch_name in arch_list:
        if arch_name in data_dict:
            ...
            queue_sizes, performance = dp.compute_performance(data_dict[arch_name], arch_name, operation)
            ax.plot(queue_sizes, performance, f"{style['marker']}-", color=style['color'], 
...
def plot_resource_comparison(ax, data_dict, arch_list, title=None):
    for arch_name in arch_list:
        if arch_name in data_dict:
            ...
            queue_sizes, resource_utilization = dp.compute_resource_utilization(data_dict[arch_name])
            ax.plot(queue_sizes, resource_utilization, f"{style['marker']}-", color=style['color'], 
                   label=style['display_name'], linewidth=2)
...
def plot_efficiency_comparison(ax, data_dict, arch_list, operation, title=None):
    ...
    for arch_name in arch_list:
        if arch_name in data_dict:
            ...
            ax.plot(queue_sizes, efficiency, f"{style['marker']}-", color=style['color'], 
                   label=style['display_name'], linewidth=2)
    
    ax.set_xlabel('Queue Size')
    ax.set_ylabel(f'Resource Cost / Performance')
...
def create_comparison_plots(data_dict_dict, output_path=None):
    ...
    fig.suptitle("Hardware Queue Architecture Comparison", fontsize=32, y=0.97)  # Adjusted y position
    ...
    for arch_name, data_dict in data_dict_dict.items():
        style = get_arch_style(arch_name)
        queue_sizes, lut_percentages = dp.get_lut_utilization(data_dict)
        axs[1, 0].plot(queue_sizes, lut_percentages, f"{style['marker']}-", color=style['color'],
                      label=style['display_name'], linewidth=2)
    ...
    for arch_name, data_dict in data_dict_dict.items():
        style = get_arch_style(arch_name)
        queue_sizes, reg_percentages = dp.get_register_utilization(data_dict)
        axs[1, 1].plot(queue_sizes, reg_percentages, f"{style['marker']}-", color=style['color'],
                      label=style['display_name'], linewidth=2)
    ...
    for arch_name, data_dict in data_dict_dict.items():
        ...
        if any(bram_percentages):  # Only plot if there's actual BRAM usage
            axs[1, 2].plot(queue_sizes, bram_percentages, f"{style['marker']}-", color=style['color'],
                          label=style['display_name'], linewidth=2)
    ...
    plot_performance_comparison(axs[2, 0], enqueue_data_dict, enqueue_arch_list, operations[0])  # Enqueue performance
    plot_performance_comparison(axs[2, 1], data_dict_dict, arch_list, operations[1])  # Dequeue performance
    plot_performance_comparison(axs[2, 2], data_dict_dict, arch_list, operations[2])  # Replace performance
    
    # Resource efficiency comparisons for different operations
    plot_efficiency_comparison(axs[3, 0], enqueue_data_dict, enqueue_arch_list, operations[0])  # Enqueue efficiency
    plot_efficiency_comparison(axs[3, 1], data_dict_dict, arch_list, operations[1])  # Dequeue efficiency
    plot_efficiency_comparison(axs[3, 2], data_dict_dict, arch_list, operations[2])  # Replace efficiency
...
def create_multi_arch_plot(plot_function, data_dict_dict, operation=None, title=None, output_path=None):
    """
    ...
    for arch_name, data_dict in data_dict_dict.items():
        if operation:
            ...
            plot_function(ax, data_dict, title=title, arch_name=arch_name, operation=operation)
        else:
            # Regular metric plot
...
def process_and_plot_all(base_dir, output_dir=None):
    ...
    for arch_dir in os.listdir(base_dir):
        ...
        results_dirs = [d for d in os.listdir(arch_path) if 'vivado_analysis_results' in d]
